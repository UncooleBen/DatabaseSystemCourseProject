...
public class UserDaoImpl extends DatabaseService implements UserDao {
    ...
    @Override
    public List<User> queryAllUsers(int skip, int limit) {
        Connection connection = getConnection();
        assert connection != null;
        List<User> result = new ArrayList<>();
        String SELECT = "SELECT * FROM t_user ORDER BY id ASC LIMIT ?, ?";
        try {
    ...
    @Override
    public int queryTotalUserNumber() {
        Connection connection = getConnection();
        assert connection != null;
        int result = 0;
        String SELECT = "SELECT COUNT(*) FROM t_user";
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(SELECT);
            ResultSet rs = preparedStatement.executeQuery();
            if (rs.next()) {
